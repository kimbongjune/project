...
<play_body>
<form id="wrdfrm" name="wrfrm" method="post" action="hotel_write.jsp">
	...
	<div id="#content01">
				...
				<div class="mainWrap">
					<form name="travelSearchForm" id="travelSearchForm" action="../Search.do<%= param %>" method="get">
						<div class="cultureSearch">
							...
							<input type="text" class="cultureBox" name="k" value="<%= k %>" placeholder="찾으시는 숙박업소가 있습니까?" autofocus>
							<input type="submit" class="cultureBtn" name="cultureBtn" value="" style="border:0px;">
						</div>
					</form>
				</div>
				<!-- 검색 끝 -->
		...
		<div class="con_area"> 
			...
			<div class="month_best">
				<hr id="hotel_listhr1">
				<h3>이달의 <span class="red">추천</span> 숙박업소</h3>
				<h4>전북의 숙박을 매달 추천해드립니다. 잠자리가 편하면 여행이 더 즐거워집니다!</h4>
			
				<!-- 탑3 -->
				<div class="month_list">
					<%
					for(int i = 0 ; i < hotellist.Total() ; i++)
					{
						ItemVo item = hotellist.Get(i);
					%>
					<div class="monthWrap">
						<a href="hotel_view.jsp?boardtype=3&page=<%= curpage %>&t=<%=t %>&no=<%= item.getNo() %>"><!-- 링크 넣을곳  -->
						...
						<p><br><!-- 동넣을곳 --><span class="stay_cate"><%= item.getNickname() %></span><!-- 숙박인지 먹는곳인지 -->
							<a href="hotel_view.jsp?boardtype=3&k=<%= k %>&no=<%= item.getNo() %>&page=<%= curpage %>"><%= item.getTitle() %></a><!-- 제목 -->
							<br>
			...
			<ul class="order">
				<%if(order.equals("1")){ %><li><a href="hotel_list.jsp?t=<%= t %>&k=<%= k %>&order=11" <% if(order.equals("1")){ out.print("class='on'"); }%>>날짜순</a></li><% }else{%>
					<li><a href="hotel_list.jsp?t=<%= t %>&k=<%= k %>&order=1" <% if(order.equals("11")){ out.print("class='on2'"); }%>>날짜순</a></li><%}%>
				<%if(order.equals("2")){ %><li><a href="hotel_list.jsp?t=<%= t %>&k=<%= k %>&order=22" <% if(order.equals("2")){ out.print("class='on'"); }%>>인기순</a></li><% }else{%>
					<li><a href="hotel_list.jsp?t=<%= t %>&k=<%= k %>&order=2" <% if(order.equals("22")){ out.print("class='on2'"); }%>>인기순</a></li><%}%>
				<%if(order.equals("3")){ %><li><a href="hotel_list.jsp?t=<%= t %>&k=<%= k %>&order=33" <% if(order.equals("3")){ out.print("class='on'"); }%>>가나다순</a></li><% }else{%>
					<li><a href="hotel_list.jsp?t=<%= t %>&k=<%= k %>&order=3" <% if(order.equals("33")){ out.print("class='on2'"); }%>>가나다순</a></li><%}%>
			</ul>
			</div>
			<%if(maxrecord == 0)
				{%>
			...
			<div class="serchList">
				...
				%>
				<div class="photoWrap">
					<a href="hotel_view.jsp?boardtype=3&t=<%=t %>&k=<%= k %>&no=<%= item.getNo() %>&page=<%= curpage %>">
						<% String body = item.getBody().replace("\""," ' ");
						if (body.indexOf("<img") != -1) 
						{
							String temp = body.substring(body.indexOf("<img"));
					...
					<h3>
						<a href="hotel_view.jsp?boardtype=3&k=<%= k %>&no=<%= item.getNo() %>&page=<%= curpage %>">
			...
			<div id="count">
				<ul>
					<li><a href="hotel_list.jsp?page=<%=1 %>&t=<%= t %>&k=<%= k %>">&lt;&lt;</a></li><!-- 첫페이지가기 -->
					<%
					if( curpage > 1)
					{
					%>
						<li><a href="hotel_list.jsp?type=<%= type %>&page=<%= curpage - 1%>&k=<%= k %>&t=<%= t %>">&lt;</a></li><!-- 앞페이지가기 -->		
					<%
					}else
					{
					...
					for(int i = startpageno; i <= endpageno; i++) 
					{
						...
						//현재 페이지 빨간색으로 강조할부분
							%>
								<li class="page"><a href="hotel_list.jsp?t=<%= t %>&k=<%= k %>&page=<%= i %>"><strong><%= i %></strong></a></li>
							<%
							continue;
						}
					%>
						<li class="page"><a href="hotel_list.jsp?t=<%= t %>&k=<%= k %>&page=<%= i %>"><%= i %></a></li>
					<%
					}
					if( curpage < maxpage)
					{
						%><li><a href="hotel_list.jsp?t=<%= t %>&k=<%= k %>&page=<%= curpage + 1 %>">&gt;</a></li><%
					}else
					{
					...
					%> 	
					<li><a href="hotel_list.jsp?t=<%= t %>&k=<%= k %>&page=<%= maxpage%>">&gt;&gt;</a></li>
				</ul>
			</div>
			<!-- 페이징 끝 -->
		</div>
