			...
			<div class="sch_left">
				<div class="sch_col">
					...
					{ ItemVo item = notice_list.Get(i);%>
						<li>
							<p>
								<a class="highlight2" href="../jjnotice/notice_view.jsp?boardtype=1&no=<%= item.getNo() %>"><%= item.getTitle() %></a>
							</p><br>
				...
				<div class="sch_col">
					...
					{ ItemVo item = play_list.Get(i);%>
						<li><strong><a class="highlight2" href="../jjplay/jj_view.jsp?boardtype=2&no=<%= item.getNo() %>"><%= item.getTitle() %></a></strong>
				...
				<div class="sch_col">
					...
					{ ItemVo item = hotel_list.Get(i);%>
						<li><strong><a class="highlight2" href="../jjinn/hotel_view.jsp?boardtype=3&no=<%= item.getNo() %>"><%= item.getTitle() %></a></strong>
							<br><span class="highlight2"><%= index._index(item.getReplacebody()) %></span>
						</li>
					<%} %>
					</ul>
				...
				<div class="sch_col">
					...
					{ ItemVo item = com_list.Get(i);%>
						<li><strong><a class="highlight2" href="../jjfree/com_view.jsp?boardtype=4&no=<%= item.getNo() %>"><%= item.getTitle() %></a></strong>
...
if(search != "")
{
	if(searchvalue == 2)
	{
		localStorage.setItem("key1", search);
	}else if(searchvalue == 3)
	{
		localStorage.setItem("key2", search);
	}else if(searchvalue == 4)
	{
		localStorage.setItem("key3", search);
	}else
	{
		localStorage.setItem("key1", localStorage.key3);
		localStorage.setItem("key3", localStorage.key2);
		localStorage.setItem("key2", search);
	}
}
...
{    
    ...
    List.unshift(localStorage[localStorage.key(i)]);
    searchList.unshift(localStorage[localStorage.key(i)]);
    //var helper = document.getElementsByClassName('ui-autocomplete ui-front ui-menu ui-widget ui-widget-content ui-corner-all');
    //helper.innerHTML = "<li class='ui-menu-item' role='presentation'><a id='ui-id-1' class='ui-corner-all' tabindex='1' style='display:block'>" + localStorage[localStorage.key(i) + "</a></li>"];
}
...
if($(".sch_txt").val() == "" || $(".sch_txt").val() == null)
{
    $(".sch_txt").autocomplete({						//자동검색어 시작
    	source: searchList,									// source는 data.js파일 내부의 List 배열
    	minLength: 0,
